...
assessmentButton.addEventListener(
  ...
  () => {
    ...
    //  診断結果表示エリアの作成
    resultDivision.innerText = '';//divタグを空文字で上書きすることで

    // headerDivision の作成
    const headerDivision = document.createElement('div'); //divタグの作成
    headerDivision.setAttribute('class', 'card-header text-bg-primary'); //headerDivisionにclassを設定
    headerDivision.innerText = '診断結果'; //タグの内側のテキストを設定

    // bodyDivision の作成
    const bodyDivision = document.createElement('div');
    bodyDivision.setAttribute('class', 'card-body'); //bodyDivisionにclassを設定

    const paragraph = document.createElement('p');//pタグの作成
    paragraph.setAttribute('class', 'card-text'); //paragraphDivisionにclassを設定
    const result= assessment(userName);//診断結果の作成
    paragraph.innerText = result;//pタグの内側の文章を設定
    bodyDivision.appendChild(paragraph);//divタグの子要素としてpタグを設定
    ...
    anchor.innerText = 'Tweet #あなたのいいところを投稿する';//ボタンの文章の追加
...
function test() {
  ...
  console.log('同じ名前なら、同じ診断結果を出力することのテスト');
  ...
  console.log('同じ名前なら、同じ診断結果を出力することのテスト終了');
}
